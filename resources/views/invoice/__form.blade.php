<div class="row">
        ...
        <div class="form-group">
            ...
            <select class="form-control custom-select {{ $errors->has('collaborator_id') ? 'is-invalid' : '' }}" name="collaborator_id" id="collaborator_id" required>
                ...
                @foreach($collaborators as $collaborator)
                    <option value="{{ $collaborator->id }}" {{ old('collaborator_id', $invoice->collaborator_id) == $collaborator->id ? 'selected' : ''}}>{{ $collaborator->name }}</option>
                @endforeach
            </select>
    ...
    <div class="col-md-6">
        <div class="form-group">
            ...
            <input type="date" class="form-control {{ $errors->has('expiration_at') ? 'is-invalid' : '' }}" name="expiration_at" id="expiration_at" value="{{ old('expiration_at', $invoice->expiration_at) == '' ? now()->addDays(config('invoices.expiration_days'))->toDateString() : $invoice->expiration_at }}" required>
